...
# Função para calcular gradiente e plano tangente
def calcular():
    ...
    ax.quiver(float(ponto_x.evalf()), float(ponto_y.evalf()), float(ponto_z.evalf()), float(grad_x.evalf()), float(grad_y.evalf()), float(grad_z.evalf()), color="green", length=1, normalize=True, label="Gradiente")
